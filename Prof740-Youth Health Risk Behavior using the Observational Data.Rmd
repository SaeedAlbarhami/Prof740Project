...
# library(skimr)
# library(kableExtra)
# skim(data) %>% skimr::kable(format = "latex", booktabs = T, caption = "Sample of observations with Empty values")
```
...
Here and overview of the dataset cleaned data from any Na observation
...
Summary of all Demographic variables that shows the level of each variable in dataset
...
```{r datasummary}
#Data Summary
pander(summary(dataOverview), caption = "Data summary table")
```
...
```{r dataOverview,  fig.width=10,fig.height=11}
...
```{r tablegroups, echo= FALSE, fig.width=10,fig.height=11}
...
## Density Distribution (Demographics).

```{r,  fig.width=10,fig.height=11 ,fig.cap= "Density plot"}

cleanedData<-omitNaForSpecificCols(dataOverview, c("sex","race","grade","age"))
par(mfrow=c(2,2))
plot(density(unclass(cleanedData$age)), xlab = "Age",main = "ssksss")
...
# 
# DataExplorer::plot_str(cleanedData)
# 
# DataExplorer::plot_missing(cleanedData)
# 
# DataExplorer::plot_density(cleanedData)
# 
# DataExplorer::plot_bar(cleanedData)

library(SmartEDA)
pander(ExpCTable(cleanedData))
pander(ExpData(data, type = 2))
