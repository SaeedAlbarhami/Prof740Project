...
## Objective:
The main objective of this project is to examine the associations between victimization, substance use, and suicide attempt among youth using the Youth Health Risk Behavior Survey data for the year 2013. We will be using Apriori (an algorithm for frequent item set mining and association rule learning) and Decision Tree. This study has several  addressing association of youth mental health with substance use, suicide attempt, and victimization. Data sample includes only youth, and thus focuses on health related outcomes not limited to substance use, suicide, or victimization. Most importantly, by using a representative sample of youth who were not age-cohorted, The data set will allow for identification of a number of potentially valuable insights.
...
#library(skimr)
#library(kableExtra)
#skim(data) %>% skimr::kable( booktabs = T, caption = "Sample of observations with Empty values")
# 
# library(pander)
...
# Data Preprocessing (Demographics)

This section will implement data cleaning process for all non needed varibles. 
...
- This function will used to clean data in two phases, phase (1) will handle the demographic variables, and phase (2) handles the other victimization variables, suicide attempt variables and 13 substance use variables.
...
New dataset has a 1514 obs. out of 5 demographic variables (age,sex,grade,race), and non empty observations, and levels have been changed into categorical (texts) for analysis in the next steps. 
...
#str(dataOverview)
library(SmartEDA)
#pander(ExpData(dataOverview,type=1))
#Data Summary
...
Here shows a sample of dataset after cleaning, and transforming the variables.
...
Here shows a statistical summary of all grouped demographic variables, this give a meaning how well are our data variables are distributed, and if data set is biased towared a certain varible. A biased dataset in many cases can effect all the anlaysis results and give a non relastic insights.

- Summary:

Sex groups variables are normaly distributed.

Race groups variables are normaly distributed.

Age groups variables are normaly distributed.

Grade groups are skwedtowards 12th, and 11th observations.
...
This section  invistigate the relations between the diffrent variables of dataset and to visulize intresting insights from data.


### Demographic Details (Age,Race,Sex, & Grade) of Respondents.


Barplot measure the distribution of variables over the dataset, it can be usefull to indicate several informative insights such as the skewness of data, how observations are domininant to others.

- Summary:

It can be seen clearly Hispanic/Latinos, and Black/African Americans are dominants when compared to other races, most of data samples are aged above 14 years old, Male and females are distributed normally among all ges in dataset
...
This section invistigate the victimizations variables in dataset, which are ranged in vicitimizations groups variables, visulizing thoses variables gave us the following insights.
...
This section invistigates the substance use variables in dataset, which are ranges in Substance use groups variables, visualizing thoses variables gave us the following insights.
...
This section invistigates the substance use variables in dataset, which are ranges in suicide attemps groups variables,visualizing thoses variables gave us the following insights.
...
We checked the number of missing values in each field using the following function that generates the following insights.
...
As a first step Here we clean all dataset of any non needed variable, and invistigate if we still have any missing value.
...
Correlation used in EDA to  measures the strengths of association between two variables., the value of the correlation coefficient varies between +1 and -1. 1.0 (a perfect positive correlation) and -1.0 (a perfect negative correlation). A 0 indicates no association between ranks. spearman correlation chioce came from the non-linear relation between the variable
...
Students Who have been on substance use are high level of sucide thoughts, and attempts.


Unsafe students are on high level to be exposed to drugs substance.



students who have been builled, have an experince or E-builled.



```{r spearman, fig.asp = .70, fig.cap= "Spearman corrplot "}
...
Here we combined all the victimization variable and combine them into one binary encoded for each one.
...
Here a last step before implement the model, aggregated columns herewill be converted into a transactional form to apply appriori rules analysis.

- Summary:

Most frequent transactional items are the highest in SubstanseUse, Then victimizarion, And  Last suicideAttempts.
...
Here will include all variables except demographics one, to see what are most frequentset.

- Summary:

Most frequent items are student who some marijuana, then who drinks more than one time in last 30 days, and last are students who considered suicide.
...
This section will implement the usage of several algoritms, on our dataset, for for mining frequent item sets apriori and ecalt algorithms will used as a model to analyize our encooded transactional dataset as a part of our unsupervised learning section, apriori and Ecalt are one of the most used algorithms to mine such a type of dataset to represent transactions lists and to filter closed and maximalitem sets, the second part will go through a supervised learning model of desicion tree  to understand what influences suicide attempts as a target variable in dataset. models parameter will be set as the default settings.
...
### Create additional model based oneR
```{r }
# #Create A Model
# dtModel<-rpart(Grade~substanceUse+victimization+suicideAttempt,trainData,method = "class",
#            control=rpart.control(minsplit=20, minbucket=1, cp = 0.001))
...
Listing Machine that have been used for the project, operating system, R version, And used libraries with their versions for future repreducbility of the project.
